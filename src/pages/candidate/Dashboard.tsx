    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button asChild variant="ghost" size="sm"><Link to="/candidate/jobs">See all</Link></Button>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {jobs.slice(0, 3).map((j) => (
            <Card key={j.id} className="p-5 hover:shadow-md transition">
              ...
              <p className="text-sm mt-2 flex items-center gap-1 text-muted-foreground"><MapPin className="h-3.5 w-3.5" />{j.location}</p>
              ...
              <Button asChild size="sm" className="mt-4 w-full rounded-xl"><Link to={`/candidate/jobs/${j.id}`}>View</Link></Button>
            </Card>
          ))}
        </div>
      </Card>
